...
- Suppose returns $r_{1},\cdots,r_{T}$ are independent draws from a normal $N(\mu,\sigma^2)$ distribution. 
- Let $m =$ sample mean and $s =$ sample std dev 
...
- $m$ is distributed normal: $N(\mu,\sigma^2/n)$
- $\frac{(n-1)s^2}{\sigma^2}$ is distributed chi-squared: $\chi^2(n-1)$.


## Confidence Intervals

- Example: $T=25$, $m=0.12$, $s=0.30$.
...
- Can we sample prices more frequently to get better estimates?
- If we sample monthly, weekly, $\ldots$ then we have more data points, so estimates are more accurate.
- Higher frequency is better for standard deviations, correlations, covariances, and betas.
- However, we lose the accuracy gains for the mean when we convert to annual terms.
- Let's see a simulation to show this.
...
``` {.p code-line-numbers=1-4|5-9|11-15|17-19|20-26}
import numpy as np
import pandas as pd
