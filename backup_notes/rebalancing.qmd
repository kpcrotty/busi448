...
## Drift in Portfolio Weights

For the moment, let's assume that the traditional 60-40 stock-bond portfolio allocation is optimal for some investor.  The investor holds 60% of their wealth in stocks and 40% of their portfolio in bonds. 
...
Let's consider what happens after returns are realized over the next few years.  @fig-portfolio-drift shows the evolution of the 60-40 portfolio in expected return-standard deviation space for five years of realized returns.  Suppose returns for the stock fund are 12.3% over the first year and the bond fund returns 5.9% over the same period.  Absent rebalancing, the portfolio weight for asset $i$ evolves following:$$ w_{i,t+1} = w_{i,t}\frac{1+r_{i,t}}{1+r_{p,t}} $$ where $r_{p,t}=\sum_{j=1}^N w_{j,t} r_{j,t}$ is the time $t$ realized portfolio return for an $N$ asset portfolio.  Thus, the portfolio going into the second year is weighted 61.4% and 38.6% in stocks and bonds respectively.  In the second year, stocks have a bad year, returning -22.8% while bonds experience a -3.6% return.  Ex post, the investor experienced greater losses than had they rebalanced back to 60-40 after the first year's returns since they were overweight stocks prior to poor performance by stocks.

Rebalancing will not always lead to better ex post outcomes over short investment time frames.  The portfolio after year 2 is weighted 56% stocks and 44% bonds, and over the third year, stocks earn a -4.8% return while bonds return 5.3%.  In this case, the investor is happy (after the fact) to have been underweight stocks.  Indeed, for this example, stocks continue to underperform bonds in years 4 (-5.5% for stocks vs 6.7% for bonds) and 5 (-6.2% for stocks vs 0.3% for bonds).  This results in the portfolio shifting further to the left on the efficient frontier and continued underallocation to stocks.

It is important to note that even though non-rebalanced portfolios that are ex ante suboptimal may do well in a small sample due to lucky draws as in the example above, they are still dominated by the tangency portfolio *prior* to the realization of uncertain returns.  The hoverdata in @fig-portfolio-drift shows the *ex ante* Sharpe ratio of the portfolio each year.   The non-rebalanced portfolios have lower Sharpe ratios than does the tangency portfolio.  

```{python}
#| label: fig-portfolio-drift
#| fig-cap: Portfolio drift from optimal portfolio
#| echo: false
import numpy as np
import pandas as pd
import plotly.graph_objects as go
...
How might this look over a 30-year investment horizon? @fig-rebalance-ts-wgts shows one possible path of portfolio weights if one did not rebalance each year.  For this set of realized returns, the stock fund performed relatively poorly over the first half of the sample, so its weights decreased from the initial 60% allocation.

```{python}
#| label: fig-rebalance-ts-wgts
#| fig-cap: Portfolio drift over time
#| echo: false
# Package/inputs
import numpy as np
import pandas as pd
...
@fig-rebalance-ts-sharpe shows how the ex ante Sharpe ratio evolves for this portfolio absent rebalancing.    

```{python}
#| label: fig-rebalance-ts-sharpe
#| fig-cap: Portfolio drift over time
#| echo: false
...
For the particular 30 years of realized returns, the realized Sharpe ratio is -0.0277 for a rebalanced portfolio versus -0.09 for the non-rebalanced portfolio.  The investor would have done better by rebalancing every year.  How representative is this particular realization of 30 years of returns?  The rebalanced portfolio usually outperforms the non-rebalanced portfolio when we simulate many 30 year samples for the stock-bond setting.  The distribution of the differences in **realized** Sharpe ratios is shown in @fig-rebalance-sharpe-sim.

```{python}
#| label: fig-rebalance-sharpe-sim
#| fig-cap: Sharpe ratios of rebalanced versus non-rebalanced portfolios
#| echo: false
numsims = 1000
...
In the example above, the reductions in Sharpe ratio are relatively modest, but, this need not be the case.  For instance, with more than two risky assets, the non-rebalanced portfolios may not even lie on the efficient frontier.  An example of this is shown in the animation in @fig-rebalance-animation for a portfolio of three assets.

```{python}
#| label: fig-rebalance-animation
#| fig-cap: A realization of portfolio evolution absent rebalancing
#| echo: false
import numpy as np
import pandas as pd
import plotly.graph_objects as go
...
Note that the examples above assume returns are iid each period and that the risk-free rate is constant.  Under these assumptions, the optimal portfolio is the same each period.  If returns are not iid, either due to time-varying expected returns, volatilities, and/or correlations, we may still need to rebalance, albeit to different optimal portfolios each period.  However, as we'll see in future chapters, estimating time-varying inputs is not always an advisable endeavor.

## Practical Frictions and Rebalancing

### Taxes {.unnumbered}
Unless an investor has additional capital to newly invest, rebalancing entails selling securities that have done relatively well and buying assets that have done relatively poorly.  In practice, these trades are not costless.  If the assets are held in a taxable account, selling   securities that have done well will result in capital gains taxes, so the potential benefit of improved portfolio allocation needs to be weighted against potential t cmnax exposure of selling overweighted assets.  
n
hnb Tbo the extent an investor has holdings in tax-advantaged accounts, tax consequences of rebalancing may be reduced or eliminated by rebalancing with trades in these acch h hgounts.  Some investors may regularly have new capital to invest.  For instance, individuals that save and invest a regular amount each month can use this new capital to rebalance their portfolio.  To do so, the new capital can be invested in the underweight securities.


### Transactions Costs {.unnumbered}

Similarly, trading entails transactions costs, either through trading fees or execution costs.  If the expected costs of the rebalancing transactions are greater than improvement in expected performance, then an investor should wait to rebalance.